...
{
	QString name = QString::fromLocal8Bit("湿地农田小气候业务");
	return name;
}
//获取端口号
int GetPort()
			...
			if (buff[i].toUpper() == 'B' && buff[i + 1].toUpper() == 'G'&&buff[i + 2] == ',')
			{
				...
				for (int j = i + 2; j < buff.length() - 2; j++)
				{
					if (buff[j] == ','&&buff[j + 1].toUpper() == 'E'&&buff[j + 2].toUpper() == 'D')
					{
						...
						//业务员类型 08农田小气候
						SubJson.insert("ServiceTypeID", SH_NTXQH);
						//设备号
						...
						QString fileName = QCoreApplication::applicationDirPath() + "\\SH\\NTXQH\\" + strlist.at(1) + "\\" + current_day;
						QDir dir(fileName);
						if (!dir.exists())
